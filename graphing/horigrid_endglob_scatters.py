...
def plot_scatter(index, ):
    ...
    x = grid_land[:,index]
    y = grid_main[:,index]
    x_endemic = np.ma.masked_where(endemics != '"endemic"', x)
    y_endemic = np.ma.masked_where(endemics != '"endemic"', y)
    x_global  = np.ma.masked_where(endemics == '"endemic"', x)
    y_global  = np.ma.masked_where(endemics == '"endemic"', y)
    # grid_land/10000 to rescale the range
    ax.scatter(x_endemic/10000, y_endemic/10000, c=colours, alpha=0.5, marker='^')
    ax.scatter(x_global/10000 , y_global/10000,  c=colours, alpha=0.5, marker='o')
    ax.set_aspect(1)
    ax.set_xlim(0, 38.1)
    ax.set_ylim(0, 38.1)
